...
# 原始 prev_seg 文件夹
prev_seg_folder = './prev_seg'  # 修改为你的路径
# 保存可视化后的文件夹
output_folder = './prev_seg_visual'
...
# 遍历文件夹
for filename in os.listdir(prev_seg_folder):
    if filename.endswith(('.png', '.jpg', '.tif')):
        filepath = os.path.join(prev_seg_folder, filename)
...
print(f'彩色可视化 prev_seg 图像已保存到 {output_folder}')